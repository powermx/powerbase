...
export async function handler(chatUpdate) {
  ...
  try {
	...
	if (!isNumber(user.messageCount)) user.messageCount = 0;
        if (!user.lbars) user.lbars = '[▒▒▒▒▒▒▒▒▒]';
        if (!user.job) user.job = 'Desempleo';
        if (!user.premium) user.premium = false;
		if (!user.deshabilitado) user.deshabilitado = false;
        if (!user.premium) user.premiumTime = 0;
		if (!user.vipcoins) user.vipcoins = 0;
		if (!user.comandos) user.comandos = 0;
		if (!user.mazmorra) user.mazmorra = 0;
		if (!user.mazmorra) user.banco = 0;
...
${tradutor.texto1[1]} ${messageNumber}/3
 ...
 ${tradutor.texto1[4]}`.trim();
		...
		if (plugin.deshabilitado == true && _user.deshabilitado == false) { // Butuh daftar?
          fail('deshabilitadop', m, this);
          continue;
        }
        m.isCommand = true;
...
case 'add':
  if (chat.welcome && !chat?.isBanned) {
    ...
    for (const user of participants) {
      let pp = 'https://raw.githubusercontent.com/powermx/dl/master/arc.jpg';
      try {
      ...
      if (userPrefix && chat.antiArab && botTt.restrict && isBotAdminNn) {
        ...
        await m.conn.sendMessage(id, {
          text: `*[❗] @${user.split('@')[0]} ᴇɴ ᴇsᴛᴇ ɢʀᴜᴘᴏ ɴᴏ sᴇ ᴘᴇʀᴍɪᴛᴇɴ ɴᴜᴍᴇʀᴏs ᴀʀᴀʙᴇs...*`,
          mentions: [user]
        }, { quoted: fkontak2 });
        return;
      }
      ...
      // ✅ Enviar descripción por privado
      //const desc = groupMetadata.desc?.toString() || 'Sin descripción';
      //await m.conn.sendMessage(user, { text: `📄 *Reglamento de grupo:*\n\n${desc}` });
    }
  }
  break;
...
case 'remove':
  for (const user of participants) {
    console.log(`${user.split('@')[0]} Saliendo`);
  }
  break;
...
export async function callUpdate(callUpdate) {
    ...
    let vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;PowerMX 👑;;;\nFN:PowerMX 👑\nORG:PowerMX 👑\nTITLE:\nitem1.TEL;waid=5212296668633:+521 229 666 8633\nitem1.X-ABLabel:PowerMX 👑\nX-WA-BIZ-DESCRIPTION:[❗] ᴄᴏɴᴛᴀᴄᴛᴀ ᴀ ᴇsᴛᴇ ɴᴜᴍ ᴘᴀʀᴀ ᴄᴏsᴀs ɪᴍᴘᴏʀᴛᴀɴᴛᴇs.\nX-WA-BIZ-NAME:PowerMX 👑\nEND:VCARD`
    await this.sendMessage(nk.from, { contacts: { displayName: 'PowerMX 👑', contacts: [{ vcard }] }}, {quoted: callmsg})
...
global.dfail = async (type, m, conn) => {
  try {
    ...
    const mensajes = {
      ...
      deshabilitadop: 'Comando *Deshabilitado*',
    };

    const msg = mensajes[type];
    if (!msg) return;

    const imagen1 = fs.readFileSync('./Menu.png'); // Asegúrate de que la imagen exista
    ...
    m.reply('⚠️ Error al enviar el mensaje de advertencia.');
  }
};
