...
mobile: MethodMobile, 
browser: opcion == '1' ? ['MX', 'Safari', '2.0.0'] : methodCodeQR ? ['MX', 'Safari', '2.0.0'] : ['Ubuntu', 'Chrome', '20.0.04'],
auth: {
creds: state.creds,
...
if (reason == 405) {
await fs.unlinkSync("./MX/" + "creds.json")
...
setInterval(async () => {
  ...
  const bio = `[ PowerMX ] Uptime: ${uptime}`;
...
import { canalId, numeroDestino, blogPath, powergp, vicentelopez } from './configVars.js';


function extractAllImagesFromHTML(html) {
  const matches = [...html.matchAll(/<img[^>]+src=["']([^"']+)["']/gi)];
  return matches.map(m => m[1].replace(/\/s\d+\//, '/s1600/'));
}
